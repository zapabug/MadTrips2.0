...
// Remove 'use client' - not standard in Vite/React unless using specific frameworks
...
interface Activity {
  ...
  price: number;
  description: string;
  category: string; // Make category required to match usage
}
...
const CustomPackageBuilder: React.FC = () => {
  const [selectedDuration, setSelectedDuration] = useState<string>(durationOptions[1].id);
  const [selectedAccommodation, setSelectedAccommodation] = useState<string>(accommodationOptions[1].id);
  ...
  useEffect(() => {
    ...
    // Round price to avoid floating point issues, e.g., to 8 decimal places for BTC
    setTotalPrice(parseFloat(price.toFixed(8)));
  ...
  const getAccommodation = (): string => {
    return accommodationOptions.find(a => a.id === selectedAccommodation)?.name || '';
  };

  const getSelectedActivitiesDetails = (): Activity[] => {
    return selectedActivities.map(activityId => {
      for (const category of activityCategories) {
        ...
        if (activity) {
          return {
            ...activity,
            category: category.name // Add category name for summary display
          };
        }
      }
      return null;
    }).filter((activity): activity is Activity => activity !== null); // Type guard to ensure not null
  };
      ...
      <div className="w-full lg:w-2/3 space-y-8">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-sand/20 dark:border-gray-700 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {accommodationOptions.map((option: AccommodationOption) => (
              <button
                key={option.id}
                className={`p-3 rounded-lg border ${
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  {option.pricePerNight > 0 && (
                    <span className="text-bitcoin font-semibold">{option.pricePerNight.toFixed(4)} BTC/night</span>
                  )}
                </div>
              </button>
            ))}
        ...
        <div className="space-y-6">
          {(activityCategories as ActivityCategory[]).map((category) => (
                ...
                <ul className="mt-2 space-y-2">
                  {getSelectedActivitiesDetails().map((activity, index) => (
                    <li key={index} className="flex justify-between text-sm">
                      ...
                      <span className="text-bitcoin">{activity.price.toFixed(4)} BTC</span>
                    </li>
                  ))}
                </ul>
              )}
