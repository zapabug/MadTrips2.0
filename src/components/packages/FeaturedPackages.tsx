            ...
            <article key={pkg.id} className="flex flex-col items-start group">
              <div className="relative w-full h-48 rounded-2xl overflow-hidden">
                <img
                  ...
                  alt={pkg.title}
                  className="object-cover w-full h-full transition-all duration-300 group-hover:scale-105 rounded-2xl"
              ...
              <div className="max-w-xl w-full">
                ...
                <div className="group relative">
                  <h3 className="mt-3 text-lg font-semibold leading-6 text-white group-hover:text-[#F7931A] transition-colors">
                    <Link to={`/packages/${pkg.id}`} aria-label={`View details for ${pkg.title}`} className="focus:outline-none focus:ring-2 focus:ring-[#F7931A] focus:ring-offset-2 rounded-sm">
                      {pkg.title}
                    </Link>
                  </h3>
                  ...
                  <ul className="mt-4 space-y-2" aria-label={`Features of ${pkg.title}`}>
                    {pkg.includes.map((feature: string, index: number) => (
